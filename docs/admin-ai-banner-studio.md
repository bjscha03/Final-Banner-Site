...
Uploaded logos and up to three photos remain separate original-image overlays. The model does not redraw these assets. Transparent outer padding is trimmed, then logos are resized proportionally into an intentional safe-area footprint even when the supplied logo is small. The generator is told the exact reserved footprint and explicitly avoids empty logo badges or replacement marks. Logo size and corner controls recomposite the original logo without another image-model call or repeated visual inspection. Retained logo/reference thumbnails are visible outside the upload disclosure. A restored logo is intentionally retained until removed.
