...
September 10 logo and latency release: default generation moved from the April GPT Image 2 snapshot to the pinned GPT Image 2.5 Flare snapshot at the same high-quality setting. Temporary background/master saves run concurrently and client polling checks every two seconds. Logo-only changes bypass model access probing, image generation, and repeated AI visual review; the prior completed visual inspection is reused while dimensions, ratio, edge coverage, and resolution are checked again. Transparent wordmarks receive a tight contrast backing so generated lettering cannot show through the original logo. Release suite: 132 tests passed; production build, focused types, and scoped lint passed. The broader repository test discovery still contains unrelated environment-dependent failures and is not the release gate.
