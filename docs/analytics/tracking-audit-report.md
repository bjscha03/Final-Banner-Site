...
- The retired seasonal campaign pages, cart items, proof flows, payment logic, emails, and order-processing branches were removed. Old customer URLs redirect to the custom-banner page, one logic-free Edge tombstone returns 410 Gone for every old function name, and stale cart items are purged and rejected server-side.
