...
- The retired seasonal campaign pages, cart items, proof flows, payment logic, emails, and order-processing branches were removed. Old customer URLs redirect to the custom-banner page, old function names are unconditional 410 Gone tombstones with no payment or database logic, and stale cart items are purged and rejected server-side.
