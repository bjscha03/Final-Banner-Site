...
- The retired seasonal campaign pages, cart items, proof routes, payment functions, emails, and order-processing branches were removed. Old customer URLs redirect to the custom-banner page, retired function URLs return 404 instead of the SPA shell, and stale cart items are purged and rejected server-side.
