  ...
  return <details className="mt-4 rounded-xl border border-slate-200 p-3">
    ...
    <fieldset disabled={busy} className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2 disabled:opacity-50">
      ...
      {!imageLayer && <>
        ...
        <label className="text-sm font-semibold">Font<select className={input} value={layer.font === 'DejaVu Serif' || layer.font === 'Georgia' ? 'DejaVu Serif' : 'DejaVu Sans'} onChange={event => change({ font: event.target.value })}>{FONTS.map(font => <option key={font}>{font}</option>)}</select></label>
    ...
    <p className="mt-2 text-xs text-slate-500">Applies to your current artwork. No new image generation. Placement stays inside the print-safe edges.</p>
  </details>;
}
