  ...
  return <div className="mb-3" data-ai-artwork-help>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg">
        ...
        <a className="min-h-11 rounded-lg border border-slate-300 px-4 py-3 text-center font-semibold" href={`mailto:support@bannersonthefly.com?subject=${encodeURIComponent(`Artwork help for ${widthIn} x ${heightIn} inch banner`)}`}>Email artwork support</a><p className="text-xs text-slate-600">Include your artwork and what you need help with. You can return to your banner while you wait.</p><div className="space-y-2 text-sm text-slate-700"><p>Upload a PNG, JPG, or PDF. Your image keeps its proportions when it is uploaded.</p><p>Show entire design keeps every edge visible. Cover banner fills the space and may crop the edges. Unlock free resize only when you want to adjust width and height independently.</p><p>Open Adjust artwork to pinch, drag, or use the zoom and position buttons.</p><p>We check printability before production and contact you if needed. No separate proof is sent.</p></div>
