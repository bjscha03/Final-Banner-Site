...
it('reviews the same totals in a modal and returns to checkout without replacing entered details', async () => {
  ...
  try {
    await act(async () => root.render(<><input aria-label="Checkout email" defaultValue="customer@example.com" /><CheckoutReviewDialog items={[]} prices={new Map()} totals={{subtotalCents:8100,discountAmountCents:1620,shippingLabel:'Shipping',taxCents:389,totalCents:6869}} /></>));
