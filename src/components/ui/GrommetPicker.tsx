...
export function GrommetPicker({
    ...
    const handleClickOutside = (event: MouseEvent) => {
      if (triggerRef.current && !triggerRef.current.contains(event.target as Node)) {
        ...
        if (isMobile) {
          ...
          // For desktop, check if click is outside the dropdown
          const dropdown = document.querySelector('[data-grommet-dropdown]');
  ...
  const renderMobileSheet = () => (
    <div className="fixed inset-0 z-[100] flex items-end" style={{ touchAction: 'none', pointerEvents: 'auto' }}>
      {/* Backdrop */}
      <div
  ...
  const renderDesktopDropdown = () => (
    <div
      data-grommet-dropdown
      className="fixed bg-white border border-gray-200 rounded-xl shadow-xl z-[100] overflow-hidden max-h-[60vh] overflow-y-auto min-w-[300px]"
      style={{
