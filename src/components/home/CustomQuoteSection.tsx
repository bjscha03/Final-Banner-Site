...
const CustomQuoteSection: React.FC = () => (
  <section className="bg-white px-4 pb-14 pt-2 sm:px-6 sm:pb-16 lg:px-8 lg:pb-20">
    <div className="mx-auto max-w-7xl border-l-4 border-[#FF6A00] bg-[#EEF4F7] text-[#0B1F3A]">
      <div className="grid gap-9 p-7 sm:p-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:p-14">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#A63C00]">Outside the standard configurator?</p>
          ...
          <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">Use the custom quote form for unusual dimensions, large quantities, specialized finishing, or an order that needs extra production planning.</p>
        ...
        <ul className="divide-y divide-slate-300 border-y border-slate-300 text-sm text-slate-700">
          {['Special sizes or configurations', 'Bulk quantities across product types', 'Artwork, finishing, and deadline notes'].map((item) => (
            <li key={item} className="flex items-center gap-3 py-4"><Check className="h-5 w-5 flex-none text-[#C94E00]" aria-hidden="true" />{item}</li>
          ))}
        </ul>
      </div>
    </div>
