...
export const AdminAIOptionsPanel: React.FC<any> = ({widthIn,heightIn,setWidthIn,setHeightIn,material,setMaterial,quantity,setQuantity,finishing,setFinishing,priceLabel,onGenerate,generating,onEdit,editPrompt,setEditPrompt,onRevert,canRevert,onDebug}) => (
  ...
  <select value={material} onChange={(e)=>setMaterial(e.target.value)} className="w-full bg-black/50 border border-white/20 rounded p-2"><option value="13oz">13oz Standard Vinyl</option><option value="15oz">15oz Premium Vinyl</option><option value="18oz">18oz Heavy Duty Vinyl</option></select>
  ...
  <button onClick={onDebug} className="w-full border border-white/30 rounded p-2">Run Debug Check</button>
  </section>
);
