  ...
  const handleAddToCart = useCallback(() => {
    ...
    if (isCarMagnet) {
      if (!uploadedFile) {
        toast({
          title: 'Artwork required',
          description: 'Please upload your Car Magnet artwork before adding to cart.',
          variant: 'destructive',
        });
        return;
      }
        ...
        <div className="max-w-[1600px] mx-auto">
                ...
                <aside className="customize-sidebar">
                  <div className="sticky-config-card sticky top-[100px] rounded-2xl border border-slate-200 bg-white p-5 shadow-sm min-h-[420px]">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600"><div className="text-sm font-semibold text-slate-900 mb-2">1. Choose your size</div>
                      ...
                      <div className="text-sm font-semibold text-slate-900 mt-4 mb-2">3. Quantity</div><div className="flex items-center gap-3 mb-4"><button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-8 h-8 flex items-center justify-center border border-gray-200 rounded-lg">-</button><input type="number" min={1} max={999} value={quantity} onChange={e => setQuantity(Math.max(1, +e.target.value || 1))} className="w-16 border rounded-lg px-2 py-1 text-sm text-center" /><button onClick={() => setQuantity(q => Math.min(999, q + 1))} className="w-8 h-8 flex items-center justify-center border border-gray-200 rounded-lg">+</button></div><div className="text-sm font-semibold text-slate-900 mt-4 mb-2">4. Finishing options</div>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
                        <button type="button" onClick={() => { setFinishingType('grommets'); setGrommets(grommets === 'none' ? 'every-2-3ft' : grommets); setPolePockets('none'); setAddRope(false); }} className={`text-left border rounded-xl px-2.5 py-2 ${finishingType === 'grommets' ? 'border-orange-500 bg-orange-50' : 'border-gray-200 bg-white'}`}><div className="text-xs font-semibold">Grommets</div><div className="text-[11px] text-green-600">Included Free</div></button>
                        <button type="button" onClick={() => { setFinishingType('pole_pockets'); setPolePockets(polePockets === 'none' ? 'top' : polePockets); setGrommets('none'); setAddRope(false); }} className={`text-left border rounded-xl px-2.5 py-2 ${finishingType === 'pole_pockets' ? 'border-orange-500 bg-orange-50' : 'border-gray-200 bg-white'}`}><div className="text-xs font-semibold">Pole Pockets</div><div className="text-[11px] text-green-600">+$2.00 / linear ft</div></button>
                        <button type="button" onClick={() => { setFinishingType('rope'); setAddRope(true); setGrommets('none'); setPolePockets('none'); }} className={`text-left border rounded-xl px-2.5 py-2 ${finishingType === 'rope' ? 'border-orange-500 bg-orange-50' : 'border-gray-200 bg-white'}`}><div className="text-xs font-semibold">Rope in Welded Hem</div><div className="text-[11px] text-green-600">+$2.00 / linear ft</div></button>
                      </div>
                      {finishingType === 'grommets' && <select value={grommets} onChange={e => setGrommets(e.target.value)} className="w-full border rounded-lg px-2 py-1.5 text-xs mb-2">{DESIGN_GROMMET_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>}
                      {finishingType === 'pole_pockets' && <select value={polePockets === 'none' ? 'top' : polePockets} onChange={e => setPolePockets(e.target.value)} className="w-full border rounded-lg px-2 py-1.5 text-xs mb-2"><option value="top">Top Only</option><option value="bottom">Bottom Only</option><option value="top-bottom">Top & Bottom</option></select>}
                      {finishingType === 'rope' && <select value={ropePlacement} onChange={e => setRopePlacement(e.target.value as RopePlacement)} className="w-full border rounded-lg px-2 py-1.5 text-xs mb-2"><option value="top">Top Only</option><option value="bottom">Bottom Only</option><option value="top-bottom">Top & Bottom</option></select>}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-[minmax(0,1fr)_420px] xl:grid-cols-[minmax(0,1fr)_460px] gap-6 xl:gap-8 max-w-full">
            <div className="space-y-6 min-w-0 max-w-full">
              ...
              {!NEW_BUILDER_ACTIVE && <ConfigCard step={isCarMagnet ? 2 : 3} title="Quantity" id="quantity-section">
              ...
              {!NEW_BUILDER_ACTIVE && <ConfigCard step={isCarMagnet ? 3 : 4} title={isCarMagnet ? 'Rounded Corners' : 'Finishing options'} id="options-section">
