...
async function renderPdfPageToCanvas(
  ...
  console.log('📄 PDF render scaling:', {
    ...
    willStretchToFill: true
  });
  
  // ALWAYS stretch to fill the entire canvas (no letterboxing)
  // Use non-uniform scaling to fill both dimensions
  const scaledViewport = page.getViewport({ scale: 1 });

  context.save();
  // Scale X and Y independently to fill the entire canvas
