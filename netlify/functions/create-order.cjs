...
function hasPreviewAdminCookie(event) {
  ...
  return String(cookieHeader).split(';').map((part) => part.trim()).includes('botf_preview_admin=1');
}
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (isAdminDeployPreviewTest) {
      ...
      const previewAdminCookieAccepted = isDeployPreview && hasPreviewAdminCookie(event);
      if (!previewAdminCookieAccepted) {
        return {
          statusCode: 403,
          headers,
          body: JSON.stringify({
            error: 'ADMIN_TEST_ORDER_NOT_AUTHORIZED',
            message: 'Preview admin access is required for Deploy Preview test checkout.',
            details: {
              isDeployPreview,
              previewAdminCookieAccepted,
            },
          }),
        };
      }
