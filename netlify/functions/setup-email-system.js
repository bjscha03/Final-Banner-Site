...
exports.handler = async (event, context) => {
  ...
  try {
    console.log('Starting email system database setup');
    const results = [];
    ...
    const tableCheck = await sql`
      ...
      AND table_name IN ('password_resets', 'email_events', 'email_verifications')
      ORDER BY table_name
    `;
    results.push(`verified tables: ${tableCheck.map(t => t.table_name).join(', ')}`);
    ...
    return {
      ...
      headers,
      body: JSON.stringify({
        message: 'Email system database setup completed successfully',
        status: 'completed',
        results: results,
        tables: tableCheck
      }),
    ...
    console.error('Email system setup error:', error);
    
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({
        error: 'Email system setup failed',
