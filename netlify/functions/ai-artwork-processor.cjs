...
exports.handler = async (event) => {
  ...
  const sql = neon(process.env.VITE_DATABASE_URL || process.env.NETLIFY_DATABASE_URL);  try {
    const { 
      orderId, 
      orderItems, 
...
async function processAIArtwork({ orderItem, orderId }) {
  ...
  // CRITICAL FIX: Update database with print-ready file URL
  try {
    console.log(`📝 Updating order item ${itemId} with print-ready file URL`);
    await sql`
      UPDATE order_items 
      SET file_key = ${printReadyResult.secure_url}
      WHERE id = ${itemId}
    `;
    console.log(`✅ Successfully updated order item ${itemId} file_key`);
  } catch (dbError) {
    console.error(`❌ Failed to update order item ${itemId} file_key:`, dbError);
    // Continue processing - don't fail the entire operation for DB update issues
  }
  return {
    orderItemId: itemId,
    success: true,
...
async function calculateTargetDimensions({ widthIn, heightIn, sourceUrl, sourcePublicId }) {
  ...
  console.log(`📏 Aspect ratios - Source: ${sourceAspectRatio.toFixed(3)}, Target: ${targetAspectRatio.toFixed(3)}, Diff: ${aspectRatioDiff.toFixed(3)}`);
  console.log(`�� Upscaling factor needed: ${maxUpscaling.toFixed(2)}x`);
