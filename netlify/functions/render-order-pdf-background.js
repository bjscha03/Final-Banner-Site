/**
 * Netlify Function: render-order-pdf-background
 * Generates press-ready PDF from banner design (background job)
 */
...
/**
 * Update job status in Cloudinary
 */
async function updateJobStatus(statusPublicId, updates) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      stream.end(Buffer.from(JSON.stringify(newStatus)));
    });
    
    console.log('[PDF Background] Status updated:', updates.status || updates.message);
  } catch (error) {
    console.error('[PDF Background] Failed to update status:', error);
  }
}
...
async function fetchImage(urlOrKey, isFileKey = false) {
  ...
  if (isFileKey) {
    ...
    await updateJobStatus(statusPublicId, { progress: 30, message: 'Image downloaded, processing...' });
    await updateJobStatus(statusPublicId, { progress: 30, message: 'Image downloaded, processing...' });
...
exports.handler = async (event) => {
  console.log('[PDF] === Starting PDF render request ===');
    await updateJobStatus(statusPublicId, { status: 'processing', progress: 10, message: 'Starting PDF generation' });
    await updateJobStatus(statusPublicId, { status: 'processing', progress: 10, message: 'Starting PDF generation' });
  ...
  try {
    ...
    await updateJobStatus(statusPublicId, { progress: 40, message: 'Upscaling image...' });
    await updateJobStatus(statusPublicId, { progress: 40, message: 'Upscaling image...' });
    ...
    await updateJobStatus(statusPublicId, { progress: 70, message: 'Creating PDF...' });
    await updateJobStatus(statusPublicId, { progress: 70, message: 'Creating PDF...' });
    ...
    await updateJobStatus(statusPublicId, { 
      status: 'complete', 
      progress: 100, 
