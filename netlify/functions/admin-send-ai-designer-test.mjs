...
const DISCOUNT_CAMPAIGN = 'ai_designer_test_30';
...
function providerIdempotencyKey(email) {
  const digest = crypto.createHash('sha256').update(`${CAMPAIGN_KEY}\0${email}`).digest('hex').slice(0, 40);
  return `bof-ai-designer-test/${digest}`;
}
  ...
  const rows = await sql`
    SELECT COALESCE(
     ...
     WHERE COALESCE(
       ...
       AND LOWER(TRIM(COALESCE(o.status::text, ''))) IN ('paid', 'in_production', 'shipped', 'delivered', 'fulfilled', 'refunded')
...
async function ensureAiDiscountCode(sql, email) {
  const existing = await sql`
    SELECT code, expires_at
      FROM discount_codes
     WHERE LOWER(email) = ${email}
       AND campaign = ${DISCOUNT_CAMPAIGN}
       AND used = FALSE
       AND expires_at > NOW()
     ORDER BY created_at DESC
     LIMIT 1
  `;
  if (existing.length) return String(existing[0].code).toUpperCase();

  for (let attempt = 0; attempt < 6; attempt += 1) {
    const code = `AI30-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
    try {
      const rows = await sql`
        INSERT INTO discount_codes (
          code, email, discount_percentage, single_use, used, expires_at,
          status, issued_at, campaign, created_at, updated_at
        ) VALUES (
          ${code}, ${email}, ${DISCOUNT_PERCENTAGE}, TRUE, FALSE,
          NOW() + (${DISCOUNT_DAYS} * INTERVAL '1 day'),
          'unused', NOW(), ${DISCOUNT_CAMPAIGN}, NOW(), NOW()
        )
        RETURNING code
      `;
      return String(rows[0].code).toUpperCase();
    } catch (error) {
      if (String(error?.code || '') !== '23505') throw error;
    }
  }
  throw new Error('Unable to generate a unique AI test discount code.');
}
...
async function claimSend(sql, input) {
  const rows = await sql`
    INSERT INTO marketing_email_sends (
      ...
      ${CAMPAIGN_KEY}, ${input.email}, ${input.email}, ${input.name || null},
      ${AI_DESIGNER_TEST_SUBJECT}, ${input.session.sub || null}, ${input.session.email || null},
      'processing', ${input.requestId}, ${input.providerKey}, ${input.unsubscribeTokenHash},
      1, NOW(), NOW(), NOW()
    )
    ON CONFLICT (campaign_key, normalized_email) DO UPDATE
      ...
      WHERE marketing_email_sends.status = 'error'
         OR (
           ...
           AND marketing_email_sends.last_attempt_at < NOW() - (${PROCESSING_LEASE_MINUTES} * INTERVAL '1 minute')
         )
...
export async function handler(event) {
  ...
  try {
    ...
    await marketingStore.ensureMarketingEmailSchema(sql);
    ...
    const suppression = await suppressionModule.findEmailSuppression(sql, email);
    if (suppression.suppressed) {
      return reply(409, { ok: false, suppressed: true, error: 'This customer is suppressed or unsubscribed and cannot receive promotional email.' });
    }

    const customerName = suppliedName || normalizeName(customer.customer_name) || email;
    const discountCode = await ensureAiDiscountCode(sql, email);
    const unsubscribeToken = marketingToken.createMarketingUnsubscribeToken(email, CAMPAIGN_KEY);
    ...
    if (!claim.claimed) {
      ...
      if (claim.row?.status === 'processing') {
        return reply(409, { ok: false, duplicate: true, status: 'processing', error: 'This AI test email is already sending.' });
      }
      return reply(409, { ok: false, duplicate: true, status: claim.row?.status || 'blocked', error: 'This customer cannot receive the AI test email again.' });
    }
    ...
    const siteUrl = process.env.URL || 'https://www.bannersonthefly.com';
    const emailContent = buildAiDesignerTestEmail({ customerName, discountCode, unsubscribeUrl, physicalAddress, siteUrl });
    const fromRaw = process.env.EMAIL_FROM_INFO || process.env.EMAIL_FROM || 'info@bannersonthefly.com';
    ...
    try {
      ...
      await sql`
        UPDATE marketing_email_sends
         ...
         WHERE id = ${claim.row.id}
      `;
      console.error('[admin-send-ai-designer-test] delivery failed', { sendId: claim.row.id, code: error?.name || error?.code || null });
      return reply(502, { ok: false, status: 'error', error: 'The AI designer test email could not be sent. You can safely retry.' });
    }
  } catch (error) {
    console.error('[admin-send-ai-designer-test] request failed', { code: error?.code || null, message: error?.message || String(error) });
    const status = error?.code === 'INVALID_JSON' ? 400 : error?.code === 'REQUEST_TOO_LARGE' ? 413 : 500;
    return reply(status, { ok: false, error: status === 500 ? 'Unable to send the AI designer test email.' : error.message });
  }
}
