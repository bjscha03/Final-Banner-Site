...
const GENERATION_GUARDRAIL = 'Create flat, full-bleed, print-ready banner artwork only. Do not create a banner mockup, fence, wall, room, pole, hanging banner, folded material, grommets, shadows, or real-world display scene. The image should be the actual design artwork that fills the canvas.';
...
function resolveModels(models) {
  ...
  const imageModel = models.find((m) => m.name?.includes('imagen-4.0-generate-001'))?.name?.replace('models/', '')
    || models.find((m) => m.name?.includes('imagen-4.0-ultra-generate-001'))?.name?.replace('models/', '')
    || models.find((m) => m.name?.includes('imagen-3.0-generate-002'))?.name?.replace('models/', '')
