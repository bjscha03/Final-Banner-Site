...
async function logQueued(to, subject) {
  try {
...
async function updateSent(id, providerId, errored, errText) {
  try {
    if (!id) return;
    const dbUrl = process.env.NETLIFY_DATABASE_URL || process.env.DATABASE_URL;
    if (!dbUrl) return;
...
exports.handler = async (event) => {
  ...
  const to = qs.to ? decodeURIComponent(qs.to) : 'bjscha02@gmail.com';
  ...
  try {
    ...
    await updateSent(queuedId, null, true, e && e.message ? e.message : String(e));
