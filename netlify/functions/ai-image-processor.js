...
exports.handler = async (event) => {
  console.log("🚀 AI Image Processor called - Method:", event.httpMethod);
  ...
  try {
    ...
    console.log(`🎨 Processing AI image: action=${action}`);
    console.log(`🔍 Original cleanPublicId: ${publicId}`);
    console.log(`🧹 Cleaned cleanPublicId: ${cleanPublicId}`);
    console.log(`📏 Dimensions: ${widthIn}×${heightIn}"`);
    if (!action || !publicId) {
      return json(400, { 
        success: false, 
    ...
    if (action === 'reset') {
      ...
      console.log('🔄 Resetting to original AI image');
    ...
    if (action === 'resize') {
      ...
      console.log(`🖨️ Generating print-ready version for ${widthIn}×${heightIn}"`);
      ...
      console.log(`📐 Target dimensions: ${targetWidthPx}×${targetHeightPx}px at ${targetDPI} DPI`);
      ...
      if (totalPixels > maxPixels) {
        ...
        console.log(`⚠️ Scaled down due to Cloudinary limits: ${finalWidthPx}×${finalHeightPx}px at ${actualDPI} DPI`);
      }
      ...
      console.log(`✅ Generated print-ready URL: ${printReadyUrl}`);
