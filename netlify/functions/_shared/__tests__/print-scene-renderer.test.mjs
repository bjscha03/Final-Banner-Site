...
try {
  ...
  renderer = await import('../print-scene-renderer.cjs');
} catch (err) {
  missingReason = err && err.message;
}

const maybeDescribe = pdfLib && renderer ? describe : describe.skip;

maybeDescribe('print scene v2 renderer', () => {
...
describe('print scene renderer dependency availability', () => {
  it('documents pdf-lib availability for vector embedding tests', () => {
    if (!pdfLib || !renderer) {
      console.warn('print-scene-renderer vector tests skipped: pdf-lib is not installed in this checkout.', missingReason);
    }
