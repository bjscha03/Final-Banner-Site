...
function integratedLettering(brief, editing = false) {
  return [
    ...
    brief.hasProtectedLogo || brief.hasIntegratedLogo ? logoPrompt(brief) : 'Do not invent logos, watermarks, signatures, tiny footer copy or unrelated branding.',
    !editing && (brief.hasProtectedLogo || brief.hasIntegratedLogo) && requestsLogoColors(brief) ? 'BRAND COLOR PRIORITY: sample the actual visible colors from the supplied logo and use them throughout the banner. Its palette takes precedence over conflicting inferred art direction or application defaults. Choose headline and accent colors from that palette with readable contrast; do not introduce unrelated brand colors.' : '',
